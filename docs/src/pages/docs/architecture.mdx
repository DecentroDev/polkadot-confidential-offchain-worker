...
2️⃣ **Subway Secure RPC Proxy** – Enhances security and efficiency for JSON-RPC calls.  

3️⃣ **HTTP Request Handling** – Uses the `HttpRequest` struct to securely fetch external data.  

4️⃣ **Response Processing & Storage** – Parses the API response and optionally submits it back on-chain.  
---
...
1️⃣ An event or extrinsic triggers the off-chain worker.  
